...
namespace Plugin\SlnPayment4\Controller;
...
use Plugin\SlnPayment4\Form\Type\CardType;
...
class PaymentController extends AbstractController {
     ...
     * @Template("@SlnPayment4/sln_card_payment.twig")
     */
    public function cardIndex(Request $request)
    {
        ...
        if ($methodClass == RegisteredCreditCard::class) {
            $isReCard = true;
            $mem = $this->container->get(Mem::class);
        ...
        if ($request->getMethod() == "POST") {
            ...
            if ($form->isValid()) {
                ...
                try {
                    ...
                    if ($methodClass == RegisteredCreditCard::class) {
                        if ($config->getThreedPay() == 1) {//3D決済
                            $master = new \Plugin\SlnPayment4\Service\SlnContent\Credit\ThreeDMaster();
                        } else {
                        ...
                        $this->eventDispatcher->dispatch('sln.payment.shopping.add_mem', $event);
                        $AddMem = $event->getArgument('AddMem');
                        
                        if (count($AddMem) && $AddMem[0] == 1 && $Order->getCustomer()) {
                            if($form->get('Token')->getData()) {
                                //トークン決済
                                $member = new \Plugin\SlnPayment4\Service\SlnContent\Credit\Member();
                ...
                $this->eventDispatcher->dispatch('sln.front.shopping.confirm.processing', $event);
        ...
        $this->eventDispatcher->dispatch('sln.payment.shopping.add_mem_view', $event);
    ...
    public function threeCard(Request $request) 
    {
        ...
        try {
            ...
            /* @var $paymentStatus \Plugin\SlnPayment4\Entity\OrderPaymentStatus */
        ...
        if ($statusId != OrderStatus::NEW //新規受付
     ...
     * @Template("@SlnPayment4/sln_recv.twig")
     */
