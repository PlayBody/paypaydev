...
use Plugin\SlnPayment4\Entity\PluginConfig;
...
class PluginConfigRepository extends AbstractRepository
{
    ...
    public function saverConfig(\Plugin\SlnPayment4\Entity\ConfigSubData $subData)
    {
        ...
        //メール情報設定
        $isNewMail = false;
        $template = $this->mailTemplateRepository->findOneBy(['file_name' => 'SlnPayment4/Resource/template/mail_template/cvs_order.twig']);
        if (!$template) {
            $isNewMail = true;
        }
        else {
    ...
    protected function createMailTpl()
    {
        ...
        $Mail->setFileName("SlnPayment4/Resource/template/mail_template/cvs_order.twig");
    ...
    /**
     * @return \Plugin\SlnPayment4\Entity\ConfigSubData
     */
