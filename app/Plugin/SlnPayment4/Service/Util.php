...
class Util
{
    ...
    {
        $this->getCardLogger()->addNotice($this->GetLogInfo() . "[{$mess}]");
    }
    
    public function addCvsNotice($mess)
    {
        $this->getCvsLogger()->addNotice($this->GetLogInfo() . "[{$mess}]");
    }
    
    public function addErrorLog($mess)
    {
        $this->ErrorLogger->addError($this->GetLogInfo() . "[{$mess}]");
    }
    ...
    public function setActionBasic(Basic $Basic, \Plugin\SlnPayment4\Entity\ConfigSubData $config)
    {
    ...
    public function setActionMerchantFree(Basic $Basic, \Eccube\Entity\Order $Order)
    {
        ...
        $this->eventDispatcher->dispatch('sln.payment.util.merchant_free', $event);
    }
    ...
    public function changeMemCard(Mem $mem, PluginConfigRepository $configRepository, \Eccube\Entity\Customer $Customer, \Plugin\SlnPayment4\Service\SlnContent\Credit\Member $member)
    {
        try {
            //登録済クレジットカード存在判断
    ...
    public function aesEnCode(\Plugin\SlnPayment4\Service\SlnAction\Content\Basic $requestParamHash, CryptAES $CryptAES) 
    {
        $arrEnCodeData = array();
