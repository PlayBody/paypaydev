...
use Plugin\DeliveryDate4\Repository\HolidayRepository;
use Plugin\DeliveryDate4\Form\Type\Admin\HolidayType;
use Plugin\DeliveryDate4\Form\Type\Admin\HolidaySearchType;
...
class HolidayController extends \Eccube\Controller\AbstractController
{
     ...
     * @Template("@DeliveryDate4/admin/Setting/Shop/holiday.twig")
     */
    ...
    public function edit(Request $request)
    {
        ...
        if ($form->isSubmitted()) {
            foreach ($form->get('holidays') as $formData) {
                ...
                $day = new \DateTime($data->getDate() . ' 12:00:00');
                $Holiday = $this->holidayRepository->getHoliday($day);
                if ($data->getAdd()) {
                    if ($formData->isValid()) {
                        if(!$Holiday){
