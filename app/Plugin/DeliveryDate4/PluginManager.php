...
class PluginManager extends AbstractPluginManager
{
    public function install(array $meta, ContainerInterface $container)
    {
        $file = new Filesystem();
        try {
            $file->copy($container->getParameter('plugin_realdir'). '/DeliveryDate4/Resource/template/default/Block/businessday_calendar.twig', $container->getParameter('eccube_theme_app_dir'). '/' . $container->getParameter('env(ECCUBE_TEMPLATE_CODE)'). '/Block/businessday_calendar.twig', true);
            return true;
        } catch (\Exception $e) {
            return false;
        }
    ...
    public function enable(array $meta, ContainerInterface $container)
    {
        ...
        $ymlPath = $container->getParameter('plugin_realdir') . '/DeliveryDate4/Resource/locale/messages.'.$translator->getLocale().'.yaml';
